...
async function getPdfjs() {
  if (!pdfjsPromise) {
    pdfjsPromise = (async () => {
      try {
        if (!CMAP_DIR) {
          CMAP_DIR = path.dirname(require.resolve('pdfjs-dist/cmaps/Adobe-CNS1-UCS2.bcmap'));
        }
        if (!STANDARD_FONT_DIR) {
          STANDARD_FONT_DIR = path.dirname(require.resolve('pdfjs-dist/standard_fonts/Helvetica.afm'));
        }
        if (!WORKER_SRC) {
          WORKER_SRC = require.resolve('pdfjs-dist/build/pdf.worker.mjs');
        }
        if (pdfjsLib?.GlobalWorkerOptions && WORKER_SRC) {
...
export async function extractPagesFromArrayBuffer(arrayBuffer) {
  ...
  const data = Buffer.isBuffer(arrayBuffer)
    ? arrayBuffer
  ...
  try {
    for (let index = 0; index < pdfDocument.numPages; index += 1) {
      ...
      let pdfText;
      try {
        pdfText = await extractPdfText(page);
      } catch (error) {
