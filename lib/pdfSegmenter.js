...
const OCR_DISABLED_ERROR_CODE = 'OCR_DISABLED';

let canvasModulePromise = null;
let canvasLoadError = null;

function isOcrEnvEnabled() {
  return typeof process !== 'undefined' && process.env.ENABLE_OCR === 'true';
}

async function loadCanvasSafe() {
  if (!isOcrEnvEnabled()) {
    const error = new Error('ocr_disabled');
    error.code = OCR_DISABLED_ERROR_CODE;
...
async function extractOcrText(page, options = {}) {
  if (!isOcrEnvEnabled() || options.enableOcr !== true) {
    const disabledError = new Error('ocr_disabled');
    disabledError.code = OCR_DISABLED_ERROR_CODE;
    throw disabledError;
  }
...
export async function extractPdfText(bufferOrArrayBuffer, options = {}) {
  let pdfjsLib;
  try {
  ...
  const ocrOptions = { enableOcr: options.enableOcr === true };
  ...
  try {
    for (let index = 0; index < pdfDocument.numPages; index += 1) {
      ...
      if (cidRatio > 0.3 && ocrOptions.enableOcr) {
        try {
          const ocrText = await extractOcrText(page, ocrOptions);
          if (ocrText.pageText) {
          ...
          } else if (error?.code === OCR_DISABLED_ERROR_CODE) {
            if (!warnedAboutCanvas) {
              console.warn('OCR fallback disabled by configuration.');
              warnedAboutCanvas = true;
            }
          } else {
