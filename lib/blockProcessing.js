...
import { ensureBucketExists, getSupabaseServerClient, uploadPartToSupabase } from './supabaseIngest.js';
...
async function normaliseUpstreamError(error, fallbackMessage = 'Upstream request failed') {
  ...
  if (error?.response) {
    ...
    try {
      if (typeof response.text === 'function') {
        ...
        if (text) {
          ...
          details.bodyPreview = details.preview;
        }
      } else if (typeof response.json === 'function') {
        const data = await response.json();
        details.data = data;
        try {
          details.bodyPreview = JSON.stringify(data).slice(0, 500);
        } catch (jsonError) {
          details.bodyPreview = String(jsonError?.message || 'Unable to serialise upstream JSON');
        }
      }
    } catch (readError) {
    ...
    if (!details.bodyPreview) {
      try {
        details.bodyPreview = JSON.stringify(error.error).slice(0, 500);
      } catch (serializationError) {
        details.bodyPreview = String(serializationError?.message || 'Unable to serialise upstream error payload');
      }
    }
  }
...
function buildDryRunEntry({ docId, window, batchIndex, batch }) {
  ...
  const hashSuffix = Buffer.from(`${docId}:${start}:${end}:${batchIndex}`)
    .toString('base64')
    .replace(/[^a-zA-Z0-9]/g, '')
    .slice(0, 10);
  const safeHash = hashSuffix || 'dry';
  const partId = `${start}-${end}-dry-${String(batchIndex + 1).padStart(3, '0')}-${safeHash}`;
  const estimatedChars = batch.reduce((sum, block) => sum + estimateBlockSize(block), 0);
  return {
    part_id: partId,
    start_page: window.start,
    end_page: window.end,
    status: 'dry-run',
    relative_path: relativePath,
    estimated_prompt_chars: estimatedChars,
    prompt_block_count: batch.length,
    block_count: batch.length,
  };
}
...
async function persistPart({
  ...
  const contentBuffer = writeResult.buffer || Buffer.from(JSON.stringify(payload));
  const hashSnippet = writeResult.sha256.slice(0, 10) || 'part';
  const partId = `${start}-${end}-${String(index + 1).padStart(3, '0')}-${hashSnippet}`;
  const supabaseKey = `parts/${docId}/${partId}.json`;
  let remote = null;
  const supabaseClient = getSupabaseServerClient();
  if (supabaseClient) {
    try {
      if (!supabaseBucketEnsured) {
  ...
  const rowCount = payload.blocks.reduce(
    (count, entry) => count + (Array.isArray(entry.rows) ? entry.rows.length : 0),
    0,
  );
  const manifestEntry = {
    ...
    part_index: index,
    part_count: payload.partTotal,
    block_count: payload.blocks.length,
    row_count: rowCount,
    source_block_ids: payload.sourceBlockIds,
    supabase_key: remote?.path,
    supabase_bucket: remote?.bucket,
    supabase_public_url: remote?.publicUrl || undefined,
  };
  if (onPartWritten) {
...
export async function processPagesToFiles({
  ...
  for (let i = 0; i < batches.length; i += 1) {
    ...
    const derivedRange = deriveRangeForBlocks(batch);
    const range = {
      start: Number.isFinite(derivedRange.start) && derivedRange.start > 0
        ? derivedRange.start
        : pageWindow?.start ?? derivedRange.start ?? 0,
      end: Number.isFinite(derivedRange.end) && derivedRange.end > 0
        ? derivedRange.end
        : pageWindow?.end ?? derivedRange.end ?? 0,
    };
